$(document).on('click', '.accept-button', function(e) {
    ...
    $.post('https://qb-weedshop/Click', JSON.stringify({}))
    if (PriceAmount != '' && PriceAmount != undefined && NoteAmount != '' && NoteAmount != undefined) {
        $.post('https://qb-weedshop/AddPrice', JSON.stringify({Price: PriceAmount, Note: NoteAmount}))
        $.post('https://qb-weedshop/CloseNui', JSON.stringify({}))
        ...
        $.post('https://qb-weedshop/ErrorClick', JSON.stringify({}))
    }
});
...
$(document).on('click', '.payment', function(e) {
    ...
    if (NumberId != null && Note!= null && Price != null) {
        $.post('https://qb-weedshop/PayReceipt', JSON.stringify({Price: Price, Note: Note, Id: NumberId}))
        $.post('https://qb-weedshop/CloseNui', JSON.stringify({}))
        $(".main-container").animate({"top": "-30vh"}, 250, function() {
            $('.items').html('');
            $('.main-container').css("display", "none");
